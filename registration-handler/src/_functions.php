<?php
  ...
  function import_competition_into_db( $competition_id, $mysqli )
  {
    ...
    if ( ! $error )
    {
      if ( ! $competition['error'] )
      {
        ...
        $sql = "REPLACE INTO {$db['rh']}_Main (competition_id, competition_name, competition_start_date, competition_end_date, competition_country_iso, competition_events) VALUES ('$competition_id', '$competition_name', '$competition_start_date', '$competition_end_date', '$competition_country', '$competition_events');";
  ...
  function check_imported_competitions( $competitions, $mysqli )
  {
    ...
    if ( $competitions )
    {
      $sql = "SELECT competition_id FROM {$db['rh']}_Main WHERE 1";
  ...
  function get_competition_data_from_db( $competition_id, $mysqli )
  {
    ...
    $query_results = $mysqli->query( "SELECT competition_name, competition_events, competition_registrations FROM {$db['rh']}_Main WHERE competition_id = '{$competition_id}'" );
