...
?>
      ...
      <div class="row">
        ...
        <?php if ( $competition_orders->num_rows ): ?>
          <div class="col-12 mt-3">
            <div class="card section">
              <div class="card-header section-title fw-bold">
                COMMANDES ENREGISTREES <sub id="total-amount"></sub>
              </div>
              <div class="card-body col-12">
                  ...
                  <?php if ( $competition_catalog_blocks ) :?>
                    <?php foreach ( $competition_catalog_blocks as $block_name => $block_value ): ?>
                      <span class="col-auto my-1 ms-3">
                        <label for="<?php echo $block_name; ?>"><?php echo $block_name; ?></label>
                        <input class="search-checkbox" type="checkbox" name="<?php echo $block_name; ?>" />
                      </span>
                    <?php endforeach; ?>
                    <span class="col-auto my-1 ms-3">
                ...
                <div class="row">
                  <?php while ( $result_row = $competition_orders->fetch_assoc() ): ?>
                    <div id="<?php echo "{$_GET['id']}_{$result_row['id']}"; ?>" class="placed-order col-12 col-md-6 col-lg-4 my-2">
                      <div class="card">
                        <div class="card-header p-3">
                          <span class="order-info fw-bold text-uppercase">
                            <span class="user-name">
                              <?php echo $result_row['user_name']; ?>
                            </span>
                            <sub class="order-amount">(<?php echo number_format( (float) $result_row['order_total'], 2, '.', '' ); ?>&nbsp;€)</sub>
                          </span>
                        </div>
                        <div class="card-body p-3 text-start">
                          <?php foreach ( from_pretty_json( $result_row['order_data'] ) as $block_name => $block_value ): ?>               
                            <div class="row mb-3<?php if ( $block_value['given'] ) echo ' strike'; ?>">
                              <div class="col-auto">
                                <button class="given btn btn-sm btn-outline-<?php echo $block_value['given'] ? 'success' : 'danger'; ?>"  type="button"></button>
                                <?php unset( $block_value['given'] ); ?>
                              </div>
                              <div class="card-text col">
                                <b class="fw-bold text-uppercase text-muted"><?php echo $block_name; ?>&nbsp;: </b>
                                <?php if ( $block_value ): ?>
                                  <ul>
                                    <?php foreach ( $block_value as $item_name => $item_value ): ?> 
                                      <li class="list-item m-0"><?php echo "{$item_value['qty']} x {$item_name}"; ?></li>
                                      <?php if ( isset( $item_value['options'] ) ): ?>
                                        <ul>
                          ...
                          <?php endforeach ?>
                          <?php if ( ! empty( $result_row['user_comment'] ) ): ?>
                            <div class="row py-4">
                              <div class="card-text col text-end">
                                <b class="text-uppercase text-muted fw-bold">Commentaire&nbsp;: </b>
                                <?php echo $result_row['user_comment']; ?>
                              </div>
                            </div>
                          <?php endif; ?>
                          <div class="row justify-content-end">
                            <div class="col-auto mt-2">
                              <button class="order-is-paid btn btn-sm btn-outline-<?php echo $result_row['has_been_paid'] ? 'success' : 'danger'; ?>" type="button"></button>
                            </div>
                            <div class="col-auto mt-2">
