<?php
  ...
  {   
    ...
    $query_results = $mysqli->query( "SELECT * FROM {$db['cu']}_Main ORDER BY competition_start_date, competition_name ASC" );
  ...
  {   
    ...
    $administrators_list = [];
    $sql = "SELECT administrator_login, administrator_email FROM {$db['sessions']}_AdminCredentials ORDER BY administrator_login ASC";
  ...
  function get_administrators_emails( $mysqli, $banned_login = '' )
  {
    ...
    $email_list = [];
    $sql = "SELECT administrator_email FROM {$db['sessions']}_AdminCredentials WHERE administrator_login != '{$banned_login}'";
    $query_results = $mysqli->query( $sql );
  ...
  {      
    ...
    $sql = "CREATE TABLE {$db['cu']}_{$competition_id} (
  ...
  {    
    ...
    $sql = "ALTER TABLE {$db['cu']}_{$competition_id} ADD PRIMARY KEY (id), ADD UNIQUE KEY(id)";
  ...
  /** 
   * import_competition_data(): import competition data into database
   * @param (string) competition_id: ID of the competition to import data for
   * @param (array) mysqli: database connection object
   * @return (string) error: error generated
   */

  function import_competition_data( $competition_id, $email, $mysqli )
  {   
    $error = create_competition_table( $competition_id, $mysqli ); //Create competition specific table  

    if ( ! $error )
    {
      $error = add_primary_key( $competition_id, $mysqli ); //Add primary key to newly created table 
    }
    
    if ( ! $error )
    {
       $error = insert_competition_into_db( $competition_id, $email, $mysqli ); //Create competition entry in main table   
    }

    if ( ! $error ) 
    {
      $all_administrators_email = get_administrators_emails( $mysqli );
      $error = send_creation_competition( $competition_id, $email, $all_administrators_email ); //Send creation confirmation to contact email   
    }
  ...
  function drop_competition_table_from_db( $competition_id, $mysqli )
  {
    ...
    $sql = "DROP TABLE {$db['cu']}_{$competition_id}";  
  ...
  function delete_competition_from_main_table( $competition_id, $mysqli )
  {
    ...
    $sql = "DELETE FROM {$db['cu']}_Main WHERE competition_id = '{$competition_id}'"; 
  ...
  function create_administrator_in_db( $id, $email, $mysqli )
  {
    ...
    $sql = "REPLACE INTO {$db['sessions']}_AdminCredentials ( administrator_login, administrator_password, administrator_email ) VALUE ( '{$id}', '{$encrypted_password}', '{$encrypted_email}')";
  ...
  function regenerate_administrator_password( $id, $email, $mysqli )
  {
    ...
    $sql = "UPDATE {$db['sessions']}_AdminCredentials SET administrator_password = '{$encrypted_password}' WHERE administrator_login = '{$id}'";
  ...
  function delete_administrator_from_db( $id, $mysqli )
  {
    ...
    $sql = "DELETE FROM {$db['sessions']}_AdminCredentials WHERE administrator_login = '{$id}'"; 
