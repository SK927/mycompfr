<?php
  ...
  { 
    if ( $order_data )
    {
      ...
      $sql = "REPLACE INTO ". DB_PREFIX_CU . "_{$competition_id} VALUE ('{$user_order_id}', '{$user_name_escaped}', '{$user_data['user_wca_id']}', '{$user_data['user_email']}', '{$user_order}', {$amount}, '{$user_comment_escaped}', 0, 0);";
  ...
  function get_items_amount( $competition_id, $mysqli )
  {
    ...
    { 
      foreach ( from_pretty_json( $order['order_data'] ) as $block_name => $block_value )
      {
