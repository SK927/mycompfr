<?php
  ...
  require_once '_functions-competition-data.php'; // $db is loaded here!
  ...
  function get_competition_orders( $competition_id, $mysqli )
  {
    ...
    $query_results = $mysqli->query( "SELECT * FROM {$db['cu']}_{$competition_id} ORDER BY user_name ASC" );
  ...
  {   
    ...
    $query_results = $mysqli->query( "SELECT * FROM {$db['cu']}_{$competition_id} WHERE id = '{$user_order_id}'" );
  ...
  function delete_user_order( $competition_id, $user_order_id, $mysqli )
  {
    ...
    $mysqli->query( "DELETE FROM {$db['cu']}_{$competition_id} WHERE id = '{$user_order_id}'" );
  ...
  { 
    ...
    if ( $order_data )
    {
      ...
      {
        $id = explode( '-', $key ); // $id is smthg like "b0-i1" so each array element indicates the depth of the object
        ...
        switch ( count( $id ) )
        {
          case 2: // Is an item
            ...
            break;
          case 3: // Is an option with selections
            $pos = strpos( $id[0], '_') + 1; 
            $number = substr( $id[0], 0, $pos );// Find number of current option as every option can be selected multiple times with a different value ($id is like 0_b1-i2)
            $blockId = substr( $id[0], $pos, 2 ); 
            $value = explode( '(+', $value ); // Remove potential additional cost from string
            $name = $value[0];
            $price = rtrim( $value[1], '€)' );
      ...
      if ( ! mysqli_error( $mysqli ) )
      {
        $error = send_order_confirmation( $competition_data, $user_order_id, $user_data['user_email'], $user_data['user_name'], $user_order, $amount, $user_comment, $is_edit );
      }
      else
      {
        $error = mysqli_error( $mysqli );
  ...
  function get_items_amount( $competition_id, $mysqli )
  {
    ...
    // First we create an array containing all possible items and selections names
    ...
    { 
      ...
      { 
        ...
        { 
          ...
          {
            $options[ $selection['name'] ] = 0; 
          }
        }
      }
      $items_amount[ $block['name'] ] = array_merge( $items, $options );
    }

    // Then we count every ordered items and selections
        ...
        foreach ( $block['items'] as $item_key => $item )
        {
          ...
          foreach ( $item['options'] as $option )
          {
            foreach ( $option as $selection_key => $selection ) 
            {
              ...
              if ( ! isset( $items_amount[ $block_name ][ $selection_name ] ) )
              { 
                $items_amount[ $block_name ][ $selection_name ] = 0;
              }
