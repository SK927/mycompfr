<?php
  ...
  {    
    ...
    $query_results = $conn->query( "SELECT order_data FROM " . DB_PREFIX_CU . "_{$competition_id} WHERE id = '{$order_id}';" ); 
    ...
    if ( $query_results )
    {
      ...
      if ( $conn->query( "UPDATE " . DB_PREFIX_CU . "_{$competition_id} SET order_data = '{$order_json}' WHERE id = '{$order_id}';" ) )
      {
