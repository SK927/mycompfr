function searchOrders()
{
  ...
  document.querySelectorAll( '.placed-order' ).forEach( function( order, i ) { // For each placed order
    let state = true;

    order.querySelectorAll( '.given' ).forEach( function( btn, index ) {
      state &= ( btn.classList.contains( 'btn-outline-danger' ) || checked[ index ] ); // If block is not given or if block checkbox is checked
    } );

    state &= ( order.querySelector( '.order-is-paid' ).classList.contains( 'btn-outline-danger' ) || checked[ checked.length - 1 ] ); // If order is not paid or non payed checkbox is checked
...
$( document ).on( 'click', '.delete-order', function( e )
{
  ...
  { 
    ...
    $.ajax( {
      type: 'POST',
      url: '../../src/admin/ajax-delete-order.php',
      ...
      { 
        ...
        if ( ! result.error ) 
        {
          ...
          $.ajax( {
            type: 'POST',
            url: '../../src/admin/ajax-update-items-amount.php',
...
$( document ).on( 'click', '.order-is-paid', function( e )
{
  ...
  let containerId = $( this ).closest( '.placed-order' ).attr( 'id' );
  let orderInfo = containerId.split( '_' );
  let button = $( this );
  
  $.ajax( {
    type: 'POST',
    url: '../../src/admin/ajax-toggle-paid.php',
    data: { competition_id: encodeURI( orderInfo[ 0 ] ), order_id: orderInfo[ 1 ] },
    success: function( response )
...
$( document ).on( 'click', '.given', function( e )
{
  ...
  let orderInfo = containerId.split( '_' );
  let block = button.closest( '.row .mb-3' );
  let blockName = block.find( '.text-uppercase' ).text();
  blockName = blockName.slice( 0, blockName.length - 3 );
  
  $.ajax( {
    type: 'POST',
    url: '../../src/admin/ajax-toggle-given.php',
    data: { competition_id: encodeURI( orderInfo[ 0 ] ), order_id: orderInfo[ 1 ], block_name: blockName },
    success: function( response )
    { 
      let result = JSON.parse( response );
      setStatusBar( result.text_to_display, result.error );
...
$( document ).on( 'click', '.update-list', function( e )
{
  ...
  $.ajax( {
    type: 'POST',
    url: '../../src/admin/ajax-update-competitors-from-wcif.php',
